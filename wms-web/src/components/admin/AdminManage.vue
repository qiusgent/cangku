...
export default {
  ...
  methods:{
    ...
    save(){
      /*validate() 方法是 el-form 组件提供的一个系统方法,用于触发表单验证。*/
    ...
    resetForm() {
      /*this.$refs.form.resetFields() 是 Element UI 提供的一个系统方法,用于快速重置表单数据*/
    ...
    mod(row){
      ...
      /*$nextTick确保操作在 DOM 更新之后执行，避免因为更新尚未完成而导致的问题。*/
      this.$nextTick(()=>{
        //赋值表单
        this.form.id=row.id
        this.form.no=row.no
        ...
        this.form.password=''
        this.form.age=row.age +'' //+'' 转换成string类型
