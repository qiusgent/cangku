<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="operate" label="操作" v-if="user.roleId!=2" width="150">
...
export default {
  ...
  data() {
    let checkCount1 =(rule, value, callback) =>{
      ...
      else if( this.form1.action==='2' && value> this.currentRow.count ){
  ...
  methods:{
    ...
    doMod(){
      this.$axios.post(this.$httpUrl+'/goods/updata',this.form).then(res=>res.data).then(res=>{
        console.log(res)
        if(res.code==200){
          this.$message({
            message: '操作成功！',
